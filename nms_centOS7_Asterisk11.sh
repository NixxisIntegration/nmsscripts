...
# ----------------------------------------------------------------------#
# [Prerequisities]  Must be run as root on centos 7                     #
...
# ----------------------------------------------------------------------#
# [File Version]        2.7.5                                           #
# [Nixxis Version]      3.0.0                                           #
# [Asterisk Version]    11.25                                           #
# ----------------------------------------------------------------------#
# [Last Update]         06 OCT 2023 (Smarcq)							#
#                                  										#
#                       							                    #
# ----------------------------------------------------------------------#
...
nms_install(){
	...
	# Check if os is supported CentOS.
	if rpm -qa \*-release | grep -Ei "centos" | cut -d"-" -f3 | grep -iho "7" > /dev/null 2>&1; then 
			osversion="7"
			if uname -m | grep -q "x86_64" > /dev/null 2>&1; then
				osarch="64"
				echo "Centos 7 64bit Installed" >> /var/log/nixxis/installation.log
			else
				echo "Centos 7 32bit Installed" >> /var/log/nixxis/installation.log
			fi
			
	else 
			printf '%-*s %s\n ' 10 "$RED [ ✗ Fail ] $NORMAL" "This installation of Nixxis is for CentOS 7 only"
			printf '\n\n %-25s %s\n ' " " "$RED This installation of Nixxis is for CentOS 7 only $NORMAL" >> /var/log/nixxis/installation.log
			exit
	fi
	printf '%-*s %s\n ' 10 "$GREEN [ ✓ Ok ] $NORMAL" "OS CentOS $osversion installed"
...
# Adding EPEL repository
	...
	pkgs=( "net-tools" "gcc gcc-c++" "libxml2-devel" "jq" "dos2unix" "ncurses-devel" "dialog" "curl" "curl-devel" "jwhois" "libuuid-devel" "ntp" "nano")
for pkg in ${pkgs[*]}; do
...
# additionnal packages 
	...
	pkgs=( "libuuid-devel" "kernel-devel" "bison" "openssl-devel" "jq" "xmlstarlet" "tmpwatch.x86_64" "ckermit.x86_64" "libtermcap-devel" "sox" "wireshark" "ncurses-devel")
...
for pkg in ${pkgs[*]}; do
	...
	printf '%-*s %s\r ' $col "Installing Jansson" "$NORMAL [ ... ] $NORMAL"
	printf '%-*s %s\r ' $col "Installing Jansson" "$NORMAL [ ... ] $NORMAL" >> /var/log/nixxis/installation.log
	cd /usr/src/ && wget http://www.digip.org/jansson/releases/jansson-2.11.tar.gz >> /var/log/nixxis/installation.log 2>&1
	tar -zxf jansson-2.11.tar.gz >> /var/log/nixxis/installation.log 2>&1
	cd jansson* >> /var/log/nixxis/installation.log 2>&1
	...
	make check >> /var/log/nixxis/installation.log 2>&1
	make install >> /var/log/nixxis/installation.log 2>&1
	ldconfig >> /var/log/nixxis/installation.log 2>&1
printf '%-*s %s\n ' $col "Installing Jansson" "$GREEN [ Done ] $NORMAL"
	printf '%-*s %s\n ' $col "Installing Jansson" "$GREEN [ Done ] $NORMAL" >> /var/log/nixxis/installation.log
...
printf '%-*s %s\r ' $col "Installing lighttpd lame" "$NORMAL [ ... ] $NORMAL"
	...
	if rpm -qa | grep  lighttpd > /dev/null 2>&1 -a rpm -qa | grep  lame > /dev/null 2>&1; then
		...
		yum install -y lighttpd lame >> /var/log/nixxis/installation.log 2>&1
...
# downloading the packages
	...
	wget http://downloads.asterisk.org/pub/telephony/asterisk/releases/asterisk-11.25.3.tar.gz >> /var/log/nixxis/installation.log 2>&1
	...
	tar zxvf asterisk-11.25.3.tar.gz >> /var/log/nixxis/installation.log 2>&1
...
# Applying Patches
	if [ "$config_asterisk_patch_AEOH" = "true" ] ; then
		...
		pushd /usr/src/asterisk/asterisk-11.25* >> /var/log/nixxis/installation.log 2>&1
...
printf '\n %-15s %-40s %s\n ' "$MAGENTA#####" "Installing Asterisk*" "#####$NORMAL" 
	...
	pushd /usr/src/asterisk/asterisk-11.25* >> /var/log/nixxis/installation.log 2>&1
	./configure --libdir=/usr/lib64 >> /var/log/nixxis/installation.log 2>&1
...
printf '%-*s %s\r ' $col "Copying files" "$NORMAL [ ... ] $NORMAL" 
	cp -rf /usr/src/asterisk/asterisk-11*/contrib/init.d/rc.redhat.asterisk /etc/init.d/asterisk >> /var/log/nixxis/installation.log 2>&1
...
nms_check(){
	...
	# Check if os is supported CentOS.
	if rpm -qa \*-release | grep -Ei "centos" | cut -d"-" -f3 | grep -iho "6" > /dev/null 2>&1; then 
		echo "Centos 6 Installed" >> /var/log/nixxis/installation.log
		osversion="6"
	elif rpm -qa \*-release | grep -Ei "centos" | cut -d"-" -f3 | grep -iho "7" > /dev/null 2>&1; then 
			echo "Centos 7 Installed" >> /var/log/nixxis/installation.log
			osversion="7"
		else 
			printf '%-*s %s\n ' 10 "$RED [ ✗ Fail ] $NORMAL" "Nixxis only support Centos 6 & 7"
			printf '\n\n %-25s %s\n ' " " "$RED Nixxis only support Centos 6 & 7 $NORMAL" >> /var/log/nixxis/installation.log
			exit
	fi
			printf '%-*s %s\n ' 10 "$GREEN [ ✓ Ok ] $NORMAL" "OS CentOS $osversion installed"
...
pkgs=( "gcc gcc-c++" "libxml2-devel" "ncurses-devel" "dialog" "curl" "curl-devel" "jwhois" "net-snmp" "kernel-devel" "bison" "openssl-devel" "sox" "wget" "mysql-devel" "sqlite-devel" "libtool-ltdl-devel" "samba-client" "perl-libwww-perl" "mlocate" "ntp" "rsync" "lighttpd" "php-cli" "nano" "vsftpd" "fail2ban")
	for pkg in ${pkgs[*]}; do
